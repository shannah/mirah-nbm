...
~~~
 ...
 5. Notice that the `$FXML1` annotion is placed just before the assignment of the `@label` member variable - which is inside the `initialize` method.  This differs from the Java placement outside of functions.  This is because Mirah doesn't have member declarations in the root class space like Java does.  Member vars and types are automatically created and inferred in Mirah based on the types that are assigned to them.
